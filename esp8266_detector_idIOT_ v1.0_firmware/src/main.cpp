...
void loop()
{
	...
	switch(estado_actual)
	{
		...
		case MODE_RUN:   //inicio de tarea, calcula luz ambiente e inicia el modo de operacion seleccionado
			calcular_offset();
			estado_actual=user_mode;
		break; 
		...
		case MODE_END_TASK:           //notificar el fin una tarea
			...
			if(FLAG_repetir_notificacion){  //TO-DO  -- falta implementar en telegram
				cronometroVirtual.begin(15000);
		...
		case MODE_WAIT:           //tiempo de espera para repetir notificacion
			if ( cronometroVirtual.estado()==false ){
				PRINTLN_DEBUG(F("Fin de espera"));  
				contador_parpadeos = 0;  // por si el modo es blink, para reiniciar un nuevo conteo de parpadeos
				estado_actual = user_mode;  
			}
		break;
...
void procesarTelegramas(int num_mensajes)
{
	for (int i = 0; i < num_mensajes; i++) {
		...
		//if (chat_id != ADMIN_USER && chat_id != OTHER_USER) { 
		...
		else if ( text == "/AP_reset" && ( chat_id==ADMIN_USER || chat_id==OTHER_USER) ) { 
		...
		else if (text == "/update" && ( chat_id==ADMIN_USER || chat_id==OTHER_USER) ) { 
		...
		else if ( text.equals("/red") && (chat_id == ADMIN_USER || chat_id == OTHER_USER) ) { 
		...
		else if ( (text.equals("/ver") || text.equals("/version")) && (chat_id == ADMIN_USER || chat_id == OTHER_USER) ) { 
			FLAG_version=true;
			enviar_cabecera( chat_id );
		}

		else if ( (text.equals("/ldr") || text.equals("/luz") || text.equals("/info")) && (chat_id == ADMIN_USER || chat_id != OTHER_USER) ) { 
			enviar_info_luz( chat_id );
		} 
		else if ( (text.equals("/offset") ) && (chat_id == ADMIN_USER || chat_id == OTHER_USER) ) { 
			calcular_offset(); 
		...
		else if ( (text.equals("/on") ) && (chat_id == ADMIN_USER || chat_id == OTHER_USER) ) { 
			user_mode = MODE_DETECT_ON; 
			bot.sendMessage(chat_id, "MODO: Detectar ENCENDIDO");
		}
		else if ( (text.equals("/off") ) && (chat_id == ADMIN_USER || chat_id == OTHER_USER) ) { 
			user_mode = MODE_DETECT_OFF; 
			bot.sendMessage(chat_id, "MODO: Detectar APAGADO");
		}
		else if ( (text.equals("/blink") ) && (chat_id == ADMIN_USER || chat_id == OTHER_USER) ) { 
			user_mode = MODE_DETECT_BLINK; 
			bot.sendMessage(chat_id, "MODO: Detectar PARPADEO");
		}
		else if ( (text.equals("/run") ) && (chat_id == ADMIN_USER || chat_id == OTHER_USER) ) { 
			estado_actual = MODE_RUN; 
			bot.sendMessage(chat_id, "Tarea INICIADA");
		}
		else if ( (text.equals("/stop") ) && (chat_id == ADMIN_USER || chat_id == OTHER_USER) ) { 
			estado_actual = MODE_STOP; 
		}
		else if ( (text.equals("/repeat") ) && (chat_id == ADMIN_USER || chat_id == OTHER_USER) ) { 
		...
		else if ( (text.equals("/config") )  && (chat_id == ADMIN_USER || chat_id == OTHER_USER) ) { 
			//if(estado_actual!=user_mode){ 
			enviar_configuracion(chat_id);
			//}
		}
		else if ( (text.substring(0,4) == "/cfg")  && (chat_id == ADMIN_USER || chat_id == OTHER_USER) ) { 
			if(estado_actual!=user_mode){ 
...
void enviar_lista_comandos(String usuario)
{
	...
	if( usuario==ADMIN_USER || usuario==OTHER_USER ){    
